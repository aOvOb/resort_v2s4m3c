...
<script type="text/javascript">
  $(function() { 
    $('#btn_delete').on("click", delete_proc);     // 삭제 진행, => btn_delete를 click했을시 delete_proc 호출

  });
  
  function delete_form(attachfileno) { 
    $('#btn_delete').attr('data-content', attachfileno); // 삭제할 파일 번호 임시 저장 
                                                                      // btn_delete ID를 찾아 data-content에 attachfileno를 저장
    
    msg = '파일을 삭제하시겠습니까?<br> 파일을 삭제하면 복구할 수 없습니다.<br>';
    msg += '삭제 진행시 삭제 진행버튼을 클릭하세요.';
  ...
  function delete_proc() {                       // 삭제할 파일 번호 임시 저장, 전역변수를 사용하지 않으면서 attachfileno를 전달하는 방법
...
<!--     <span class='menu_divide' > | </span> -->
  ...
  <div style='width: 100%;'>
    <table class="table table-striped" style='width: 100%;'>
      ...
      <%-- table 내용 --%>
      <tbody>
        <c:forEach var="attachfileVO" items="${list }"> <!-- class => attachfileVO, attachfileVO 객체가 존재-->
