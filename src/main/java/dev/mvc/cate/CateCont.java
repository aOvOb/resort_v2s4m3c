...
public class CateCont {
  ...
  public ModelAndView update(CateVO cateVO) {
    // CateVO cateVO <FORM> 태그의 값으로 자동 생성됨.
    // request.setAttribute("cateVO", cateVO); 자동 실행
  ...
  /**
   * 삭제폼 http;//localhost:9090/resort/cate/read_delete.do
   * 
   * @return
   */
  ...
  // http://localhost:9090/resort/cate/update_seqno_down.do?cateno=1000
  /**
   * 우선순위 하향 up 1 ▷ 10
   * @param cateno 카테고리 번호
   * @return
  ...
  public ModelAndView update_visible(CateVO cateVO) {
    ...
    if(cnt == 1) {
      ...
      mav.setViewName("/cate/update_visible_mag"); // /cate/update_visible_mag.jsp
    }
    return mav;
  }
