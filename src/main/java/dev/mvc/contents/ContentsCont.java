...
public class ContentsCont {
  ...
  public ModelAndView create(HttpServletRequest request, ContentsVO contentsVO) {
    ...
    // System.out.println("IP: " + request.getRemoteAddr()); // Oracle은 "" -> null로 인식
    ...
    int cnt = this.contentsProc.create(contentsVO); // 등록 처리
    if (cnt == 1) {
  ...
  public ModelAndView list() {
    ModelAndView mav = new ModelAndView();
    mav.setViewName("/contents/list_all"); // /webapp/contents/list.jsp
  ...
  public ModelAndView read(int contentsno) {
    ...
    System.out.println("--> 첨부파일 갯수: " + attachfile_list.size());
  ...
  public ModelAndView delete(HttpServletRequest request,
                                       int cateno, int contentsno, String passwd) {
    ModelAndView mav = new ModelAndView();
  ...
  /**
   * 메인 이미지 등록폼 http;//localhost:9090/resort/contents/img_create.do
   * 
   * @return
   */
  ...
  public ModelAndView img_create(HttpServletRequest request, ContentsVO contentsVO) {
    ...
    if (passwd_cnt == 1) { // 패스워드가 일치할 경우 파일 업로드
      ...
      mav.setViewName("redirect:/contents/read.do?contentsno=" + contentsVO.getContentsno()); // /webapp/contents/create_msg.jsp
